<template>
	<div class="center_public">
		...
		<div class="select" v-if="dataLoaded&&type==1">
			<span><i style="color:red">*</i>项目转让</span>
			<Select v-model="post.transfers" style="width:300px">
				<Option v-for="item in TradingData.transfers" :value="item.name">{{ item.name }}</Option>
			</Select>
		</div>
        <div class="select" v-if="dataLoaded&&type==1">
			<span><i style="color:red">*</i>交易属性</span>
			<Select v-model="post.attributes" style="width:300px">
				<Option v-for="item in TradingData.attributes" :value="item.name">{{ item.name }}</Option>
			</Select>
		</div>
		<div class="select" v-if="dataLoaded&&type==1">
			<span><i style="color:red">*</i>信息来源</span>
			<Select v-model="post.information" style="width:300px">
				<Option v-for="item in TradingData.lines" :value="item.name">{{ item.name }}</Option>
			</Select>
		</div>
		<div class="select" v-if="dataLoaded&&type==1">
			<span><i style="color:red">*</i>交易金额</span>
			<Select v-model="post.amounts" style="width:300px">
				<Option v-for="item in TradingData.amounts" :value="item.name">{{ item.name }}</Option>
			</Select>
		</div>
		...
		<div class="select" v-if="dataLoaded&&type==2">
			...
			<Select v-model="land" style="width:300px">
				<Option v-for="item in TradingData.lands" :value="item.name">{{ item.name }}</Option>
			</Select>
		</div>
		<div class="select" v-if="dataLoaded&&type==2">
			<span><i style="color:red">*</i>信息来源</span>
		...
		<div class="select" v-if="dataLoaded&&type==3">
			<span><i style="color:red">*</i>融资期限</span>
			<Select  v-model="post.theFinancing" style="width:300px">
				<Option v-for="item in TradingData.fundings" :value="item.name">{{ item.name }}</Option>
			</Select>
		</div>
		<div class="select" v-if="dataLoaded&&type==3">
			<span><i style="color:red">*</i>融资主体</span>
			<Select v-model="post.financeBody" style="width:300px">
				<Option v-for="item in TradingData.lines" :value="item.name">{{ item.name }}</Option>
			</Select>
		</div>
		<div class="select" v-if="dataLoaded&&type==3">
			<span><i style="color:red">*</i>信息来源</span>
			<Select v-model="post.information" style="width:300px">
				<Option v-for="item in TradingData.ways" :value="item.name">{{ item.name }}</Option>
			</Select>
		</div>
		<div class="select" v-if="dataLoaded&&type==3">
		...
		<div class="center_public_second">
            ...
            <div class="inp" v-show="type==3">
				<label><em>*</em>还款来源</label>
				<input style="border: 1px solid #eeeeee;" v-model="post.sourceRepayment" type="text" class="title">
			</div>
            <div class="inp" v-show="type==3">
				<label><em>*</em>资金用途</label>
				<input type="text" v-model="post.capitalUses" class="title">
			</div>
...
export default {
	...
	computed:{
	  ...
	  checka:function(){
		  return (this.type&&this.c_city&&this.post.amounts&&this.post.name&&this.post.title&&this.post.describe&&this.post.province&&this.post.information&&this.post.attributes&&this.post.transfers&&this.c_img)
	  },
	  checkb:function(){
		  return (this.type&&this.land&&this.c_city&&this.post.province&&this.post.title&&this.post.describe&&this.post.information&&this.c_img)
	  },
	  checkc:function(){
		  return (this.type&&this.c_city&&this.post.province&&this.post.title&&this.post.describe&&this.post.amounts&&this.post.capitalUses&&this.post.theFinancing&&this.post.sourceRepayment&&this.post.information&&this.post.financeBody&&this.c_img)
	  },
	  fileOne:function(){
	...
	watch:{
		type:{
			async handler(newValue,oldValue){
				...
				this.img_back = []
				this.fujian_back = []
				let res = await P.TradingData({"type":this.type})
	...
	methods:{
		...
		async submit(){
			...
			if(this.type==1&&this.checka){
				let res = await P.TradingProject(JSON.stringify({
					"uid":this.auth.uid+'',
					"city":this.c_city+'',
					"picture":this.c_img+'',
					"type":this.type+"",
